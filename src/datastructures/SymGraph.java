    ...
    private boolean refine(int color) {
        ...
        for (Integer integer : topnodes) {
            ...
            for (int j = 0; j < links.length; j++) {
                ...
                if (motifLink.getLinkType().getMotifLink() == motifLink) {
                    degreesTop[i][motifLink.getLinkType().getLinkTypeID()]++;
                    degreesTop[integer][nrTypes + motifLink.getLinkType().getLinkTypeID()]++;
                } else {
                    degreesTop[integer][motifLink.getLinkType().getLinkTypeID()]++;
                    degreesTop[i][nrTypes + motifLink.getLinkType().getLinkTypeID()]++;
                }
        ...
        for (Integer integer : bottomnodes) {
            ...
            for (int j = 0; j < links.length; j++) {
                ...
                MotifLink motifLink = links[j];
                if (motifLink.getLinkType().getMotifLink() == motifLink) {
                    degreesBottom[i][motifLink.getLinkType().getLinkTypeID()]++;
                    degreesBottom[integer][nrTypes + motifLink.getLinkType().getLinkTypeID()]++;
                } else {
                    degreesBottom[integer][motifLink.getLinkType().getLinkTypeID()]++;
                    degreesBottom[i][nrTypes + motifLink.getLinkType().getLinkTypeID()]++;
                }
            }
        }
